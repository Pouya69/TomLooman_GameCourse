...
#include "RogueProjectileAttackAction.h"
...
URogueProjectileAttackAction::URogueProjectileAttackAction()
{
	ActionName = "Projectile Attack";
	MuzzleSocketName = "Muzzle_01";
	
}

void URogueProjectileAttackAction::StartAction_Implementation(AActor* Instigator)
{
	...
	UNiagaraFunctionLibrary::SpawnSystemAttached(MagicCastingEffect, Character->GetMesh(), MuzzleSocketName,
	...
	{
		AttackTimerElapsed(Instigator, MagicProjectileClass);
	});
	
	GetWorld()->GetTimerManager().SetTimer(AttackTimerHandle, TimerDelegate, AttackTimer, false);	
}

void URogueProjectileAttackAction::StopAction_Implementation(AActor* Instigator)
{
...
void URogueProjectileAttackAction::AttackTimerElapsed(AActor* Instigator, const TSubclassOf<ARogueProjectileBase> ProjectileClassToSpawn)
{
	ACharacter* Character = Cast<ACharacter>(Instigator);
	if (!Character) return;
